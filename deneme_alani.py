# deneme_alani.py
import streamlit as st
from PIL import Image
from streamlit_cropper import st_cropper

def calistir():
    st.header("📐 En/Boy Kilitli Koordinat Aracı")
    st.write("Fareyle serbestçe alanı belirleyebilir, yukarıdan en/boy oranını dinamik olarak kilitleyebilirsiniz.")
    
    # 1. EN/BOY ORANI GİRİŞİ (Kullanıcı dinamik olarak değiştirebilir)
    ratio_input = st.text_input("🔒 En : Boy Oranı Kilidi (Örn: 15:17 veya serbest çizim için boş bırakın)", value="15:17")
    
    # Oran girdisini ayrıştırma mantığı
    aspect_ratio = None
    if ratio_input and ":" in ratio_input:
        try:
            parts = ratio_input.split(":")
            w_ratio = float(parts[0])
            h_ratio = float(parts[1])
            if w_ratio > 0 and h_ratio > 0:
                aspect_ratio = (w_ratio, h_ratio)
                st.caption(f"🎯 Oran **{ratio_input}** olarak kilitlendi. Fareyle boyutlandırırken bu oran asla bozulmayacak.")
        except ValueError:
            st.error("❌ Geçersiz format! Lütfen '15:17' veya '1:1' şeklinde sayı girin.")
            
    st.markdown("---")
    
    # 2. GÖRSEL YÜKLEME VE TEK EKRANDA ÇALIŞMA
    referans_mockup = st.file_uploader("Boş Mockup Yükle", type=["png", "jpg"], key="cropper_upload")
    
    if referans_mockup:
        ref_img = Image.open(referans_mockup)
        orj_genislik, orj_yukseklik = ref_img.size
        st.info(f"📋 Yüklenen Görselin Orijinal Boyutu: {orj_genislik} x {orj_yukseklik} piksel")
        
        # Cropper aracı kilitlenen oranla birlikte doğrudan çalışır
        box_coords = st_cropper(
            ref_img, 
            realtime_update=True, 
            box_color='blue', 
            aspect_ratio=aspect_ratio, 
            return_type='box'
        )
        
        if box_coords:
            st.success("🎉 Koordinatlar Hesaplandı!")
            
            # E-tabloya direkt kopyalanacak temiz blok
            st.markdown("**E-Tabloya Kopyalamak İçin Değerler:**")
            st.code(
                f"x_noktasi: {int(box_coords['left'])}\n"
                f"y_noktasi: {int(box_coords['top'])}\n"
                f"genislik: {int(box_coords['width'])}\n"
                f"yukseklik: {int(box_coords['height'])}"
            )
            
            # Değerlerin ekrandaki anlık metrik gösterimi
            col1, col2, col3, col4 = st.columns(4)
            col1.metric("x_noktasi (X)", int(box_coords['left']))
            col2.metric("y_noktasi (Y)", int(box_coords['top']))
            col3.metric("genislik (G)", int(box_coords['width']))
            col4.metric("yukseklik (H)", int(box_coords['height']))
