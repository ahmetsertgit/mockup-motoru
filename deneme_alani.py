...
def kaydet_veritabani(sheets_client, mockup_name, kategori, drive_file_id, x, y, w, h, aci):
    ...
    Açıyı da dahil ederek I sütununa kadar (9 sütun) güncelleme/ekleme yapar.
    """
    try:
        ...
        # Sütun yapısı: mockup_id, mockup_name, kategori, drive_file_id, x_noktasi, y_noktasi, genislik, yukseklik, dondurme_acisi
        if bulunan_satir:
...
def calistir(drive_service=None, sheets_client=None):
        ...
        with col_sol_gorsel:
            with st.spinner('Görsel çekiliyor...'):
                ...
                if secilen_gorsel_id in mevcut_gorsel_verileri:
                    ...
                    st.session_state.val_angle = int(v.get('aci', 0))
    ...
    # --- SOL SÜTUN (Kırpma Alanı) ---
    with col_sol_gorsel:
        st.markdown("**1️⃣ Hizalama Alanını Seçin:**")
    ...
    with col_sag_bilgi:
        ...
        if kaydet_butonu:
            basarili, sonuc_mesaji = kaydet_veritabani(
                ...
                st.session_state.val_angle
            )
            with col_msg:
                if basarili:
                    st.markdown(f'<div style="display: flex; align-items: center; justify-content: center; height: 38px; color: #155724; background-color: #d4edda; border: 1px solid #c3e6cb; border-radius: 0.25rem; font-size: 0.85rem; font-weight: 500; padding: 0 4px; text-align: center;">{sonuc_mesaji}</div>', unsafe_allow_html=True)
                    st.rerun()
                else:
                    st.markdown(f'<div style="display: flex; align-items: center; justify-content: center; height: 38px; color: #721c24; background-color: #f8d7da; border: 1px solid #f5c6cb; border-radius: 0.25rem; font-size: 0.85rem; font-weight: 500; padding: 0 4px; text-align: center;">{sonuc_mesaji}</div>', unsafe_allow_html=True)
        ...
        # --- DÖNDÜRME ALANI (SLIDER) ---
        st.markdown("---")
        st.markdown("**🔄 Baskı Döndürme Açısı:**")
        st.slider("Döndürme Derecesi (Saat Yönü)", min_value=-180, max_value=180, value=0, step=1, key="val_angle")
    ...
    # --- MATEMATİKSEL ROTASYON VE CANLI ÖNİZLEME (SOL SÜTUN ALTINDA) ---
    with col_sol_gorsel:
        st.markdown("---")
        st.markdown("**2️⃣ 🔄 Canlı Açı ve Baskı Önizlemesi:**")
        
        # Arayüzdeki güncel piksel durumlarını alıyoruz
        cx_s = st.session_state.cur_x_w_h["x"]
        cy_s = st.session_state.cur_x_w_h["y"]
        cw_s = st.session_state.cur_x_w_h["w"]
        ch_s = st.session_state.cur_x_w_h["h"]
        aci_derece = st.session_state.get("val_angle", 0)
        
        # Görselin kopyasını oluşturup üzerine çizim yapacağız
        onizleme_gorseli = st.session_state.cropper_gorseli.copy()
        cizici = ImageDraw.Draw(onizleme_gorseli)
        
        # Kutunun merkez noktasını bulalım
        merkez_x = cx_s + (cw_s / 2)
        merkez_y = cy_s + (ch_s / 2)
        
        # Dereceyi radyana çevirelim
        radyan = math.radians(aci_derece)
        cos_a = math.cos(radyan)
        sin_a = math.sin(radyan)
        
        # Köşe döndürme fonksiyonu (Saat yönü rotasyon matrisi)
        def koseyi_dondur(px, py, cx, cy, c, s):
            dx = px - cx
            dy = py - cy
            nx = dx * c - dy * s
            ny = dx * s + dy * c
        ...
        # Dikdörtgenin 4 temel köşesi
        ham_koseler = [
            (cx_s, cy_s),                 # Sol Üst
            (cx_s + cw_s, cy_s),           # Sağ Üst
            (cx_s + cw_s, cy_s + ch_s),     # Sağ Alt
            (cx_s, cy_s + ch_s)            # Sol Alt
        ]
        
        # Tüm köşeleri hesaplanan merkez etrafında döndür
        dondurulmus_koseler = [koseyi_dondur(x, y, merkez_x, merkez_y, cos_a, sin_a) for x, y in ham_koseler]
        
        # Döndürülmüş poligonu (baskı alanını) kırmızı kalın çizgiyle çiz
        cizici.polygon(dondurulmus_koseler, outline="red", width=3)
        
        # Ekrana canlı resmi bas
        st.image(onizleme_gorseli, use_container_width=False)
